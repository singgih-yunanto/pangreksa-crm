...
export function ListView({ module, q }: { module: ModuleConfig; q: string }) {
  const router = useRouter();
  const { has } = useAuth();
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } = useInfiniteList(module.endpoint, q);
  ...
  if (!isLoading && rows.length === 0) {
    return (
      <Card className="p-10 text-center">
        <p className="text-[15px] font-medium">No {module.plural.toLowerCase()} yet</p>
        <p className="text-[13px] text-muted mt-1">Create your first {module.singular.toLowerCase()} to get started.</p>
      </Card>
    );
  }
